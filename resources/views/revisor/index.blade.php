            ...
            <div class="col-8">
                <div class="card">
                    <div class="card-header">
                        <h2>{{__('ui.Annuncio #')}} {{$article->id}}</h2>
                        <hr>
                        <div class="card-body">
                            <div class="row">
        ...
        <div class="row justify-content-center">
            <div class="col-8">
                ...
                <form class="my-2" action="{{route('revisor.reject', ['id'=>$article->id])}}" method="post">
                    @csrf
                        @method('put')
                        <button type="submit" class="btn-presto">{{__('ui.Rifiuta')}}</button>
                </form>
    
                <form class="my-2" action="{{route('revisor.accept', ['id'=>$article->id])}}" method="post">
                    @csrf
                        @method('put')
                        <button type="submit" class="btn-presto">{{__('ui.Accetta')}}</button>
                </form>
                
                <form class="my-5" action="{{route('revisor.indexReject', compact('article'))}}" method="get">
                    @csrf        
                        <button type="submit" class="btn-presto">{{__('ui.Rivaluta')}}</button>
                </form>
